...
# Type of outlet
vars <- c("outlet_w_advert", "outlet_no_advert", "advert_only", "supermarket", "shop", "kiosk", "stand_table_top", "local_vendor", 
          "bar_pub", "other_outlet", "poster_adv", "onsite_adv", "painting_adv")
i <- 1
...
for (i in 1:13) {
  ...
  hold2 <- data.frame(table(accra[[vars[i]]])) # Overall prevalence of values
  hold$Overall <- hold2$Freq / 413
...
vars <- c("adv_grain_cereal", "adv_fresh_meat_poultry", "adv_fresh_fish_shellfish", "adv_procssd_fried_meat_poultry", "adv_procssd_fried_fish", 
          ...
          "adv_vegetables", "adv_cakes_sweets", "adv_savoury_snacks_pies", "adv_sodas_sweetened_bevs", "adv_milk", "adv_fresh_juices", 
          "adv_alcohol", "adv_sugar_sweet_spreads", "adv_fats_oils", "adv_legumes_pulses", "adv_condiments", "adv_eggs")
names <- c("grain_cereal", "fresh_meat_poultry", "fresh_fish_shellfish", "procssd_fried_meat_poultry", "procssd_fried_fish", "trad_mixed_dishes", 
           "modern_mixed_dishes", "soups_stews", "fried_roots_tubers_plntn_pots", "nonfried_roots_tubers_plntn_pots", "vegetables", 
           "cakes_sweets", "savoury_snacks_pies", "sodas_sweetened_bevs", "milk", "fresh_juices", "alcohol", "sugar_sweet_spreads", 
           "fats_oils", "legumes_pulses", "condiments", "eggs")

i <- 1

for (i in 1:22) {
 
  hold <- data.frame(t(lca_all5$probs[[vars[i]]])) # Select probs for single variable and transpose output (i.e. swap rows and columns around)
  colnames(hold) <- c("Class 1", "Class 2", "Class 3", "Class 4", "Class 5") # Change column names
  hold2 <- data.frame(table(accra[[vars[i]]])) # Overall prevalence of values
  hold$Overall <- hold2$Freq / 413
